...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    if (!mount || webglUnavailable) return undefined;
  ...
  if (webglUnavailable) {
    ...
    const fallbackColor = `${Math.round(r * 255)}, ${Math.round(g * 255)}, ${Math.round(b * 255)}`;
    ...
    return (
      <div
        style={{
          ...
          position: 'relative',
          filter: 'blur(8px)',
          background: `radial-gradient(120% 100% at 50% 100%, rgba(${fallbackColor}, 0.72) 0%, rgba(${fallbackColor}, 0.28) 36%, rgba(${fallbackColor}, 0.1) 58%, rgba(${fallbackColor}, 0) 80%)`,
          ...style
        }}
        className={className || ''}
      />
  ...
  return <div ref={mountRef} style={{ width: '100%', height: '100%', position: 'relative', ...style }} className={className || ''} />;
};
